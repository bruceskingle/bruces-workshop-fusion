...
CMD_ID          = f'{config.COMPANY_NAME}_{config.ADDIN_NAME}_quickSketchTextY'
CMD_NAME        = 'Y Axis Label'
CMD_Description = 'Creates a Simplex.shx text label (6mm high, 30mm wide box) in the active sketch, constrained by its right-centre to a selected sketch point.'
...
def command_created(args: adsk.core.CommandCreatedEventArgs):
    ...
    point_input = inputs.addSelectionInput(POINT_INPUT_ID, 'Anchor Point', 'Select a sketch point for the right-centre of the text')
...
def command_execute(args: adsk.core.CommandEventArgs):
    ...
    boxWidth   = 3.0    # 30mm in cm
    boxHeight  = 1.5    # 15mm in cm (enough to contain 6mm text with margin)
    ...
    extentPoint = adsk.core.Point3D.create(ptX + boxWidth, ptY + boxHeight, 0)
    ...
    # Find the right boundary line (highest average X among the 4 rectangle lines)
    # and constrain the selected point to its midpoint — right-centre of the text box
    ...
    for line in rectLines:
        avgX = (line.startSketchPoint.geometry.x +
    ...
    if rightLine:
        sketch.geometricConstraints.addMidPoint(selectedPoint, rightLine)
